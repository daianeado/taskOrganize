...
export class TarefaPage {
  ...
  incluirTarefaCalendario(descricao, datainicio, datafim){
     this.calendar.requestReadWritePermission();

      this.calendar.createEvent(descricao, null, descricao, datainicio, datafim).then(
        (msg) => {},
        (err) => {}
      );
  }

  deletarTarefaCalendario(codigoTarefa){
   let tarefas = this.tarefasService.getTarefas();

    for (let i = 0; i < tarefas.length; i++) {
      if (tarefas[i].codigo == codigoTarefa) {
        this.calendar.deleteEvent(tarefas[i].descricao, null, 
          tarefas[i].descricao, tarefas[i].data, tarefas[i].data).then(
            (msg) => { },
            (err) => { }
          );
      }
